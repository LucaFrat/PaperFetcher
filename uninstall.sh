...
# Blank the value via set-environment. Don't also call unset-environment:
# on systemd 255 that observably re-exposes the underlying value from the
# calling session's env (the very value we just blanked) instead of
# removing the entry. set-environment to empty is the only operation that
# actually leaves the manager with no real key.
